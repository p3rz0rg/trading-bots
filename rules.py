...
# ── Multi-pair crypto bot rules ─────────────────────────────
MAX_OPEN_CRYPTO      = 3         # global cap across ALL pairs
MIN_24H_VOLUME_USD   = 5_000_000 # liquidity floor: $5M/day minimum
MAX_SPREAD_PCT       = 0.002     # 0.2% max bid/ask spread


def liquidity_ok(volume_24h_usd: float, bid: float, ask: float) -> bool:
    """Liquidity filter: never touch thin markets.
    Requires healthy 24h dollar volume AND a tight spread."""
...
def crypto_slots_available(open_positions: int) -> bool:
    """Global position cap across all crypto pairs."""
    return open_positions < MAX_OPEN_CRYPTO
